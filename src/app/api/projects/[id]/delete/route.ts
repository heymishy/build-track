...
async function DELETE(
  ...
  try {
    ...
    console.log('DELETE API: User attempting delete:', { id: user.id, email: user.email, role: user.role })
    ...
    if (user.role !== 'ADMIN') {
      ...
      if (userProjectAccess.role !== 'OWNER' && user.role !== 'ADMIN') {
        console.log('DELETE API: Permission denied - User role in project:', userProjectAccess.role, 'Global role:', user.role)
        return NextResponse.json({
          success: false,
      ...
      console.log('DELETE API: User has permission to delete project - Role:', userProjectAccess.role)
    }
