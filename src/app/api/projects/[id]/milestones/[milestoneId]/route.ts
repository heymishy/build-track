...
async function PUT(
  ...
  try {
    ...
    const { name, description, targetDate, actualDate, paymentAmount, percentComplete, status, sortOrder } = body
    ...
    if (percentComplete !== undefined) updateData.percentComplete = Math.max(0, Math.min(100, Number(percentComplete)))
