    ...
    try {
      ...
      try {
        ...
        testResults.push({
          ...
          code: error.code,
          suggestion:
            error.code === 403 ? 'IAM API access restricted' : 'Service account access issue',
        })
      }
      ...
      try {
        ...
        const activeKeys =
          keys.data.keys?.filter(key => key.validAfterTime && key.validBeforeTime) || []
      ...
      try {
        ...
        const serviceAccountBindings =
          policy.data.bindings?.filter(binding =>
        ...
        testResults.push({
          ...
          suggestion:
            error.code === 403
              ? 'IAM policy access restricted by organization'
              : 'Policy access issue',
        })
      }
      ...
      try {
        ...
        testResults.push({
          ...
          suggestion:
            error.code === 403
              ? 'Sheets API blocked by domain policy'
              : error.code === 404
                ? 'Test spreadsheet not accessible'
                : 'API access issue',
        })
      }
      ...
      // Analyze patterns
      const domainPolicyIndicators = testResults.filter(
        r =>
          r.status === 'FAILED' &&
      ...
      const recommendation =
        domainPolicyIndicators.length > 0
