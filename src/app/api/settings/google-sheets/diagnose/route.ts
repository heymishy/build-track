  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const requiredApis = ['sheets.googleapis.com', 'drive.googleapis.com']
      ...
      try {
        ...
        diagnostics.tests.push({
          ...
          details: {
            code: error.code,
            suggestion:
              error.code === 403 ? 'Drive API write permissions missing' : 'API access issue',
          },
        })
        diagnostics.summary.failed++
      }
      ...
      try {
        ...
        diagnostics.tests.push({
          ...
          details: {
            ...
            suggestion:
              error.code === 403
                ? 'Sheets API not enabled or insufficient permissions'
                : 'API access issue',
          },
        })
        diagnostics.summary.failed++
      ...
      if (failedTests.some((t: any) => t.name === 'Basic Authentication')) {
        recommendations.push(
          '🔑 Service account key appears to be invalid or corrupted. Generate a new key.'
        )
      }

      if (failedTests.some((t: any) => t.name === 'Project Access')) {
        recommendations.push(
          '🏗️ Service account may be from wrong project. Ensure key matches your Google Cloud project.'
        )
      }

      if (failedTests.some((t: any) => t.name.includes('API'))) {
        recommendations.push(
          '🔧 Enable required APIs: Google Sheets API and Google Drive API in Google Cloud Console.'
        )
      }

      if (failedTests.some((t: any) => t.error?.includes('permission'))) {
        recommendations.push(
          '👤 Service account needs proper IAM roles. Assign "Editor" or "Owner" role in IAM console.'
        )
      }

      if (recommendations.length === 0) {
        recommendations.push(
          '✅ All tests passed! The issue might be intermittent or related to API propagation.'
        )
      }
