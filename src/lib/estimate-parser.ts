 ...
 */
export async function parseEstimateFromCSV(csvContent: string, filename: string): Promise<ParsedEstimate> {
 ...
 */
export async function parseEstimateFromXLSX(buffer: Buffer, filename: string): Promise<ParsedEstimate> {
 ...
 */
export async function parseEstimateFromPDF(buffer: Buffer, filename: string, userId?: string): Promise<ParsedEstimate> {
  ...
  if (tableData.length === 0) {
    throw new Error('No estimate data found in PDF. Please ensure the PDF contains tabular cost data or try using LLM parsing with valid API configuration.')
  }
...
function extractTableFromText(text: string): string[][] {
  ...
  for (const line of lines) {
    ...
    if (cells.length < 2 && cleaned.includes(',')) {
      cells = cleaned.split(',').map(cell => cell.trim()).filter(cell => cell)
    }
  ...
  if (tableData.length < 5) { // Increased threshold - we expect more data
 ...
 */
function parseRowToLineItem(row: any[], columnMap: Record<string, number>): EstimateLineItem | null {
  try {
...
function analyzeDocumentStructure(text: string) {
  ...
  const hasLabourMaterials = text.toLowerCase().includes('labour') || text.toLowerCase().includes('materials')
  const hasMarginProfit = text.toLowerCase().includes('margin') || text.toLowerCase().includes('profit')
...
function categorizeTradeDynamically(tradeName: string, documentStructure: any): string {
  ...
  if (lower.includes('aluminium') || lower.includes('joinery') || lower.includes('door')) return 'Windows & Doors'
  ...
  if (lower.includes('margin') || lower.includes('p&g') || lower.includes('profit')) return 'Project Costs'
 ...
 */
function determineCategoryFromTrade(tradeName: string): 'LABOR' | 'MATERIAL' | 'EQUIPMENT' | 'SUBCONTRACTOR' {
  const lower = tradeName.toLowerCase()
...
function trySpecializedParsing(text: string, filename: string): ParsedEstimate | null {
  ...
  if (originalLines.length === 1 && originalLines[0].length > 1000) {
    ...
    processedLines = segments
      .map(segment => segment.trim())
      .filter(segment => segment.length > 10) // Remove short fragments
  ...
  for (const line of processedLines) {
    ...
    if (lineMatch) {
      ...
      const skipPatterns = [
        'grand total', 'sub total', 'subtotal', 'total   475', 'net   413', 'gst   61'
      ]
      
      // Don't skip standalone "margin", "total", "net" as they might be legitimate line items
      const shouldSkip = skipPatterns.some(pattern => lowerTradeName.includes(pattern)) ||
  ...
  if (tradeMatches.length < 10) {
    ...
    while ((match = pattern1.exec(fullText)) !== null) {
      ...
      if (amount > 0 && amount < 500000 && tradeName.length > 3 && !shouldSkip && !seenTrades.has(key)) {
    ...
    if (documentStructure.detectedTrades) {
      documentStructure.detectedTrades.forEach(tradeName => {
        ...
        if (match) {
          ...
          const lowerTradeName = tradeName.toLowerCase()
          const shouldSkip = lowerTradeName === 'total' ||
                            lowerTradeName === 'net' ||
                            lowerTradeName === 'gst' ||
  ...
  if (tradeMatches.length === 0) {
    ...
    for (const trade of knownTrades) {
      ...
      if (match) {
        ...
        if (amount > 0 && amount < 1000000 && !seenTrades.has(key)) { // Sanity check on amount
  ...
  if (missedLargeAmounts.length > 0) {
    ...
    console.log(`💰 If we captured these, total would be: $${(totalFromMatches + totalMissed).toLocaleString()}`)
  }
  ...
  const lineItems: EstimateLineItem[] = tradeMatches.map((trade, index) => {
    ...
    switch (category) {
      ...
      default:
        ...
        if (tradeLower.includes('material') || tradeLower.includes('supplies')) {
          materialCost = trade.amount
        } else if (tradeLower.includes('equipment') || tradeLower.includes('plant') || tradeLower.includes('hire')) {
          equipmentCost = trade.amount
    ...
    return {
      ...
      category: category === 'SUBCONTRACTOR' ? determineCostCategory(materialCost, laborCost, equipmentCost) : category
    }
  })
 ...
 */
function determineCostCategory(material: number, labor: number, equipment: number): 'MATERIAL' | 'LABOR' | 'EQUIPMENT' | 'OTHER' {
