...
class MonitoringService {
  ...
  getMetrics(name?: string, fromTimestamp?: number, toTimestamp?: number): MetricData[] {
  ...
  getAggregateMetrics(
    ...
    const metrics = this.metrics.filter(m => m.name === name && m.timestamp > cutoff)
  ...
  private async checkCacheHealth(): Promise<ComponentHealth> {
    try {
      ...
      return {
        status:
          isWorking && latency < 50
            ? 'healthy'
            : isWorking && latency < 200
              ? 'degraded'
              : 'unhealthy',
        latency,
    ...
    try {
      ...
      return {
        ...
        details: hasLLMConfig
          ...
          : 'External API configuration incomplete',
      }
    } catch (error) {
      return {
        status: 'unhealthy',
    ...
    const alertConfigs: AlertConfig[] = [
      {
        ...
        condition: metrics => {
          const recent = metrics.filter(
            m => m.name === 'response_time' && m.timestamp > Date.now() - 60000
          )
          const avg =
      ...
      },
      {
        ...
        condition: metrics => {
          const recent = metrics.filter(
            m => m.name === 'error_rate' && m.timestamp > Date.now() - 60000
          )
          const avg =
            recent.length > 0 ? recent.reduce((sum, m) => sum + m.value, 0) / recent.length : 0
      ...
      },
      {
        ...
        condition: metrics => {
          const recent = metrics.filter(
            m => m.name === 'memory_usage' && m.timestamp > Date.now() - 60000
          )
