...
const DEFAULT_TRADE_RATES: Omit<TradeRate, 'id'>[] = [
  { tradeName: 'Concrete', skillLevel: 'Apprentice', hourlyRate: 28, markupPercent: 15, overheadPercent: 10 },
  { tradeName: 'Concrete', skillLevel: 'Journeyman', hourlyRate: 45, markupPercent: 15, overheadPercent: 10 },
  { tradeName: 'Concrete', skillLevel: 'Foreman', hourlyRate: 58, markupPercent: 15, overheadPercent: 10 },
  
  { tradeName: 'Plumbing', skillLevel: 'Journeyman', hourlyRate: 55, markupPercent: 20, overheadPercent: 12 },
  { tradeName: 'Plumbing', skillLevel: 'Foreman', hourlyRate: 68, markupPercent: 20, overheadPercent: 12 },
  
  { tradeName: 'Electrical', skillLevel: 'Journeyman', hourlyRate: 58, markupPercent: 20, overheadPercent: 12 },
  { tradeName: 'Electrical', skillLevel: 'Foreman', hourlyRate: 72, markupPercent: 20, overheadPercent: 12 },
  
  { tradeName: 'Carpentry', skillLevel: 'Apprentice', hourlyRate: 25, markupPercent: 18, overheadPercent: 10 },
  { tradeName: 'Carpentry', skillLevel: 'Journeyman', hourlyRate: 48, markupPercent: 18, overheadPercent: 10 },
  { tradeName: 'Carpentry', skillLevel: 'Foreman', hourlyRate: 62, markupPercent: 18, overheadPercent: 10 },
  
  { tradeName: 'Painting', skillLevel: 'Journeyman', hourlyRate: 42, markupPercent: 15, overheadPercent: 8 },
  { tradeName: 'Roofing', skillLevel: 'Journeyman', hourlyRate: 52, markupPercent: 18, overheadPercent: 10 },
  { tradeName: 'Tiling', skillLevel: 'Journeyman', hourlyRate: 48, markupPercent: 16, overheadPercent: 10 },
  { tradeName: 'Plastering', skillLevel: 'Journeyman', hourlyRate: 46, markupPercent: 15, overheadPercent: 10 },
]
...
export function TradeRatesManager() {
  ...
  const updateTradeRate = (id: string, updates: Partial<TradeRate>) => {
    setTradeRates(tradeRates.map(rate => 
  ...
  const groupedRates = tradeRates.reduce((groups, rate) => {
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">Manage hourly rates for different trades and skill levels</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-orange-50 p-4 rounded-lg">
            ...
            <div className="text-2xl font-bold text-orange-700">
              {formatCurrency(tradeRates.reduce((sum, r) => sum + r.hourlyRate, 0) / tradeRates.length || 0)}
            </div>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <div className="text-sm text-purple-600">Avg Effective Rate</div>
            <div className="text-2xl font-bold text-purple-700">
              {formatCurrency(tradeRates.reduce((sum, r) => sum + calculateEffectiveRate(r), 0) / tradeRates.length || 0)}
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewRate({ ...newRate, hourlyRate: parseFloat(e.target.value) || 0 })}
                placeholder="0.00"
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setNewRate({ ...newRate, markupPercent: parseFloat(e.target.value) || 0 })}
                placeholder="0"
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
                ...
                {rates.map((rate) => (
                  <div key={rate.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-4 flex-1">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getSkillLevelStyle(rate.skillLevel)}`}>
                        {rate.skillLevel}
                      </span>
                      ...
                      {editingId === rate.id ? (
                        <div className="flex items-center space-x-2 flex-1">
                          <input
                            ...
                            onChange={(e) => updateTradeRate(rate.id, { hourlyRate: parseFloat(e.target.value) || 0 })}
                          ...
                          <input
                            ...
                            onChange={(e) => updateTradeRate(rate.id, { markupPercent: parseFloat(e.target.value) || 0 })}
      ...
      {Object.keys(groupedRates).length === 0 && (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          ...
          <p className="text-gray-500 mb-4">Add your first trade rate to get started with hourly estimates.</p>
          <button
            onClick={() => setShowAddForm(true)}
            className="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-blue-700"
          >
