    ...
    return (
      <div className="flex items-center space-x-2">
        ...
        <div className="min-w-0">
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium text-gray-900">{config.shortLabel}</span>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Badge
                variant="secondary"
                className={`${config.textColor} ${config.bgColor} border-current`}
              >
                Active
              </Badge>
            </div>
          ...
          {showDescription && <p className="text-sm text-gray-600 mt-1">{config.description}</p>}

          {projectName && (
            <p className="text-xs text-gray-500 mt-2 font-medium">Project: {projectName}</p>
          )}
        </div>
      </div>
    </div>
        ...
        return (
          <React.Fragment key={stage}>
            ...
            >
              <Icon
                className={`w-3.5 h-3.5 ${
                  isActive ? config.textColor : isCompleted ? 'text-green-600' : 'text-gray-400'
                }`}
              />
            ...
            {index < stages.length - 1 && (
              <div
                className={`w-2 h-0.5 ${index < currentIndex ? 'bg-green-300' : 'bg-gray-300'}`}
              />
            )}
          </React.Fragment>
        )
