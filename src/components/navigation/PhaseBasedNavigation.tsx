...
export function PhaseBasedNavigation({ projects, activeView, onViewChange, loading = false }: PhaseBasedNavigationProps) {
      ...
      return [
        {
          ...
          icon: DocumentCheckIcon,
          color: 'text-blue-800',
          bgColor: currentProject.status === 'PLANNING' ? 'bg-blue-100 border-blue-400' : 'bg-blue-50 hover:bg-blue-100 border-blue-300',
        ...
        },
        {
          ...
          icon: ClockIcon,
          color: 'text-orange-800',
          bgColor: currentProject.status === 'IN_PROGRESS' ? 'bg-orange-100 border-orange-400' : 'bg-orange-50 hover:bg-orange-100 border-orange-300',
        ...
        },
        {
          ...
          icon: CheckCircleIcon,
          color: 'text-green-800',
          bgColor: currentProject.status === 'COMPLETED' ? 'bg-green-100 border-green-400' : 'bg-green-50 hover:bg-green-100 border-green-300',
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Active indicator */}
                  {isActive && (
                    <div className={`absolute bottom-0 left-0 right-0 h-1 ${item.color.replace('text-', 'bg-')} rounded-b-lg`} />
                  )}
                </button>
              )
            })}
        ...
        {useMultiProjectView && activeView !== 'all' && (
          <div className="px-6 pb-3">
            <div className="text-xs text-gray-500">
              Showing projects in <span className="font-medium">{navItems.find(n => n.key === activeView)?.label}</span> phase
              <button 
...
export function PhaseNavigationProvider({ 
  ...
  return (
    <PhaseNavigationContext.Provider value={{
      activePhase,
      setActivePhase,
      filteredProjects,
